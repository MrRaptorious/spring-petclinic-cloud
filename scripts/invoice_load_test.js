...
export default function () {
  ...
  // Optional: Wenn du den /test Endpunkt testen willst:
  // let testRes = http.get(`${BASE_URL}/test`);
  // check(testRes, {
  //   'GET /test status is 200': (r) => r.status === 200,
  //   'GET /test body contains expected string': (r) => r.body === 'invoiceRepository.findByVisitId(visitId);',
  // });
  ...
  const invoicePayload = JSON.stringify({
    ...
    id: 1,
    visitId: 1, // Die VisitId muss im Payload enthalten sein, da sie auch gesetzt wird
  ...
  check(postRes, {
    'POST /visits/{visitId}/invoice status is 200 OK': (r) => r.status === 200,
